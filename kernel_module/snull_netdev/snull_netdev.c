...
int snull_open(struct net_device *dev) {
    ...
    if (dev == snull_devs[1]) /* Another Interface */
 ...
 */
static void snull_hw_tx(unsigned char *buf, int len, struct net_device *dev)
{
    /*
...
static void snull_setup(struct net_device *dev) {
    ...
    dev->flags |= IFF_NOARP;
    dev->features |= NETIF_F_HW_CSUM; /* netdev_features.h */
...
{   
    ...
    for (ii = 0; ii < 2; ii++) {
        if (snull_devs[ii]) {
            ...
            /* Note that our internal cleanup (done in snull_teardown_pool) 
               cannot happen until the device has been unregistered. */
...
/* Insert module will call */
int snull_init_module(void) {
    int result;
    int ii;
    int ret = -ENOMEM;
    snull_interrupt = use_napi ? snull_napi_interrupt : snull_regular_interrupt;
    /* 1. Allocate network device memory */
    ...
    if (snull_devs[0] == NULL || snull_devs[1] == NULL)
        goto out;
    ret = -ENODEV;
    /* 2. Once the net_device structure has been initialized, completing the process is just a
          matter of passing the structure to register_netdev*/
    for (ii = 0; ii < 2; ii++) {
        if((result = register_netdev(snull_devs[ii]))) {
            printk("snull: Error %d registering device ... \"%s\"\n", result, snull_devs[ii]->name);
        } else {
            ret = 0;
        }
    }
